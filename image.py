...
def process_image(image_file, client, manual_command, town, incident_time,
                 output_base=None, file_prefix="i1",
    ...
    try:
        ...
        if output_base:
            ...
            image_output_filename = f"{file_prefix}{image_ext}"
            image_output_path = os.path.join(image_folder, image_output_filename)
            
            # Copy the image file
            with open(img_path, "rb") as src, open(image_output_path, "wb") as dst:
                dst.write(src.read())
        ...
        if image_output_path and os.path.exists(image_output_path):
            with col2:
                st.info(f"🖼️ Image saved: {os.path.basename(image_output_path)}")
        ...
        if image_output_path:
            output_files.append(image_output_path)
...
#     deep_analyze_image, auto_detect_news_type,
...
# def main():
#     st.set_page_config(
#         page_title="AI TV News Script Generator - Image (Telugu)",
#         layout="wide",
#         page_icon="📸"
#     )
    
#     st.title("📸 AI-Powered TV News Script Generator - Image")
#     st.caption("Upload Images → AI Understands Content → Generates Professional Telugu Broadcast Scripts")
...
#             st.success(f"✓ Image uploaded ({file_size:.1f} MB)")
...
#             with st.expander("View Analysis"):
#                 st.text_area("Analysis:", content_analysis, height=200)
...
#             selected_model = get_optimal_model("image", selected_commands, file_size_mb)
...
#             script_paths = save_tv_script_output_multilingual(tv_scripts, "image", selected_commands)
